...
async def predict_failure(request: FailurePredictionRequest):
    ...
    return FailurePredictionResponse(
        ...
        contributing_factors=[
            {"factor": "code_churn", "value": 350, "threshold": 300, "impact": "high"},
            {
...
async def detect_anomalies(pipeline_id: str, window_hours: int = 24):
    ...
    logger.info(
        "Detecting anomalies", pipeline_id=pipeline_id, window_hours=window_hours
    )
...
async def get_failure_patterns(days: int = 30):
    ...
    return {
        ...
        "patterns": [
            ...
            {"pattern": "resource_exhaustion", "occurrences": 10, "percentage": 21.3},
        ],
    }
