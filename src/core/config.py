...
class Settings(BaseSettings):
    ...
    PIPELINE_ENGINE_URL: str = Field(
        default="http://pipeline-engine:8080", env="PIPELINE_ENGINE_URL"
    )
    PIPELINE_ENGINE_API_KEY: Optional[str] = Field(
        default=None, env="PIPELINE_ENGINE_API_KEY"
    )

    # Tekton Integration
    TEKTON_NAMESPACE: str = Field(default="tekton-pipelines", env="TEKTON_NAMESPACE")
    TEKTON_DASHBOARD_URL: str = Field(
        default="http://tekton-dashboard:9097", env="TEKTON_DASHBOARD_URL"
    )

    # ArgoCD Integration
    ARGOCD_SERVER: str = Field(default="argocd-server:443", env="ARGOCD_SERVER")
    ...
    ENABLE_MODEL_EXPLANATIONS: bool = Field(
        default=True, env="ENABLE_MODEL_EXPLANATIONS"
    )

    # Performance Settings
    MAX_CONCURRENT_PREDICTIONS: int = Field(
        default=100, env="MAX_CONCURRENT_PREDICTIONS"
    )
    ...
    FEATURE_EXTRACTION_TIMEOUT: int = Field(
        default=300, env="FEATURE_EXTRACTION_TIMEOUT"
    )
