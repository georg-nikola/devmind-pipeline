...
from models.build_data import BuildData, BuildOptimizationRequest, BuildOptimizationResponse
...
class BuildOptimizer:
    ...
    async def optimize_build(self, request: BuildOptimizationRequest) -> BuildOptimizationResponse:
        """
        ...
        try:
            ...
            recommendations = await self._generate_recommendations(request, features, predictions)
    ...
    async def _extract_features(self, request: BuildOptimizationRequest) -> Dict[str, float]:
        """Extract features from build optimization request."""
        features = {}
        
        # Basic project features
        features["dependency_count"] = len(request.dependencies)
        features["code_change_size"] = request.code_changes.lines_added + request.code_changes.lines_deleted
        ...
        if request.historical_data:
            ...
            if build_times:
                ...
                features["build_success_rate"] = sum(1 for build in request.historical_data if build.success) / len(request.historical_data)
            else:
        ...
        # Dependency analysis
        features["package_size"] = await self._estimate_package_size(request.dependencies)
        features["dependency_update_frequency"] = await self._calculate_dependency_update_frequency(request.dependencies)
        
        # Infrastructure features
        features["target_environment"] = self._encode_environment(request.target_environment)
        features["parallel_jobs"] = request.build_config.get("parallel_jobs", 1) if request.build_config else 1
    ...
    async def _predict_build_metrics(self, features: Dict[str, float]) -> Dict[str, float]:
        ...
        try:
            ...
            optimization_potential = await self._estimate_optimization_potential(features, prediction)
    ...
    async def _heuristic_prediction(self, features: Dict[str, float]) -> Dict[str, float]:
        ...
        estimated_time = base_time + dependency_time + change_time + test_time + historical_factor
    ...
    async def _generate_recommendations(
        ...
        resource_allocation = await self._recommend_resource_allocation(features, predictions)
        ...
        dependency_optimizations = await self._recommend_dependency_optimizations(request.dependencies)
        recommendations["dependency_optimizations"] = dependency_optimizations
        
        # Build step optimizations
        build_optimizations = await self._recommend_build_optimizations(request, features)
        recommendations["build_optimizations"] = build_optimizations
        
        return recommendations
    
    async def _recommend_cache_strategy(self, request: BuildOptimizationRequest, features: Dict[str, float]) -> Dict[str, Any]:
    ...
    async def _recommend_resource_allocation(self, features: Dict[str, float], predictions: Dict[str, float]) -> Dict[str, Any]:
    ...
    async def train_model(self, training_data: List[BuildData]) -> Dict[str, float]:
        ...
        try:
            ...
            if len(X) < self.settings.MIN_TRAINING_SAMPLES:
                raise ValueError(f"Insufficient training data: {len(X)} samples, minimum {self.settings.MIN_TRAINING_SAMPLES}")
    ...
    async def _prepare_training_data(self, training_data: List[BuildData]) -> Tuple[np.ndarray, np.ndarray]:
    ...
    async def _calculate_branch_complexity(self, request: BuildOptimizationRequest) -> float:
        ...
        lines_changed = request.code_changes.lines_added + request.code_changes.lines_deleted
    ...
    async def _calculate_commit_frequency(self, request: BuildOptimizationRequest) -> float:
    ...
    async def _calculate_dependency_update_frequency(self, dependencies: List[str]) -> float:
    ...
    async def _estimate_optimization_potential(self, features: Dict[str, float], prediction: float) -> float:
    ...
    async def health_check(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "cache_status": await self.cache_manager.health_check() if self.cache_manager else "unavailable"
        }